/* ForgeLab — Complete MongoDB Curriculum */
(function () {
  const lessons = {};
...
<div class="callout"><strong>ForgeLab note:</strong> MongoDB 4.0+ supports multi-document transactions on replica sets. Use them when needed, but design for document locality first.</div>
...
<pre><code>db.hello.insertOne({ msg: "ForgeLab connected", at: new Date() })
...
<ol>
  ...
  <li>Use <strong>Filter</strong> bar: <code>{ msg: "ForgeLab connected" }</code> → documents match.</li>
</ol>
...
`,
  ...
  const cert = {
    intro: "Map MongoDB ForgeLab progress to MongoDB Associate Developer / Database Administrator exam themes.",
    tracks: [
